...
- **Email reliability:** Resend is integrated, but staging/production need working credentials. Some order/fulfillment callers do not act on a failed email result; no durable outbox/retry mechanism was evident.
- **Safe testing:** there is no explicit environment-wide test-payment lock or recipient allowlist. Configure or implement these before exercising workflows that could contact customers or create real payment invoices.
