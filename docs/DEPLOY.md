...
| `deploy-staging.yml`    | push to `main` (or run manually)                    | checks → **build** → built `PUBLIC_ORIGIN` must equal `STAGING_URL` → deploy guard (`scripts/deploy-guard.mjs staging`) → migrate the staging D1 → seed reference data → deploy guard again → deploy the built worker → read-only release and migration proof (`scripts/staging-migration-check.mjs`) → smoke-test `/api/health` (`"ok":true`) → prove the staging access boundary (`scripts/staging-access-check.mjs`: public pages 200 with noindex, staff pages and private APIs behind their logins). The proof requires the deployed Worker commit to equal `GITHUB_SHA` and its staging health report to show the exact migration history from the build journal. The build and the first guard run before the migration because a migration cannot be rolled back. Refuses to run until `STAGING_URL` is set, or while `STAGING_ACCESS_OPEN` exists as a Worker secret. |
...
`GET /api/health` reports `{ ok, db, docs, env, release, at }` with no caching and no business data; it is what both smoke tests poll. In staging it also reports `migration: { ok, expected, applied }`, where `expected` is the complete journal manifest compiled into the Worker and `applied` is the read-only `d1_migrations` history. `db` is `ok` when every table and column in the schema can be selected (`LIMIT 0`, so no rows are read) and `docs` is `ok` when the documents bucket answers a HEAD; otherwise each is `unavailable`. Staging `ok` is true only when both dependencies and the exact migration history are `ok`; production keeps the dependency-only check. A healthy staging answer has status 200, and a mismatch has status 503. `env` is `APP_ENV` (`unknown` when unset), `release` is the commit the build was made from (`null` when none was written in), and `at` is the time of the answer.
