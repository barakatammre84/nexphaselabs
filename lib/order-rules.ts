...
export function refundAllowed(order: { paymentStatus: string; refundDueCents: number | null; refundCents: number | null; totalCents: number }): boolean {
  return order.paymentStatus === 'refund_due' && (order.refundCents ?? 0) < refundDue(order);
}

/** Amount owed back; falls back to the order total for orders that predate the column. */
export function refundDue(order: { refundDueCents: number | null; totalCents: number }): number {
...
export function returnAllowed(order: { status: string; returnedAt: Date | null }): boolean {
  return order.status === 'shipped' && !order.returnedAt;
}

export type RefundInput = { amount: string; reference: string };
export type RefundValidation = { ok: true; amountCents: number; reference: string } | { ok: false; error: string };

/** Amount in dollars as typed, up to what is still owed; a reference is required because money moved outside this system. */
export function validateRefund(raw: RefundInput, remainingDueCents: number): RefundValidation {
  const totalCents = remainingDueCents;
  const reference = (raw.reference ?? '').trim().slice(0, 120);
  if (!reference) return { ok: false, error: 'Enter the bank or provider reference for the refund.' };
  const text = (raw.amount ?? '').trim().replace(/^\$/, '');
  if (!/^\d+(?:\.\d{1,2})?$/.test(text)) return { ok: false, error: 'Enter the refunded amount in dollars, e.g. 90.00.' };
  const amountCents = Math.round(Number(text) * 100);
  if (amountCents <= 0) return { ok: false, error: 'The refunded amount must be more than zero.' };
  if (amountCents > totalCents) return { ok: false, error: `The refund cannot exceed the $${(totalCents / 100).toFixed(2)} still owed.` };
...
export type ReturnLine = { id: string; sku: string; quantity: number; lotId: string | null; unitPriceCents: number };
export type ReturnRaw = { packs: Record<string, number>; receivedOn: string; condition: string; note: string };
export type ReturnValidation =
  | { ok: true; lines: { itemId: string; packs: number }[]; receivedOn: Date; condition: string; note: string; refundDueCents: number }
  | { ok: false; error: string };

/** Pure validation of a return: a real calendar date not before the shipment, packs within what shipped, a condition. */
export function validateReturn(items: ReturnLine[], raw: ReturnRaw, shippedAt: Date | null, now = new Date()): ReturnValidation {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(raw.receivedOn)) return { ok: false, error: 'Received date must be YYYY-MM-DD.' };
  ...
  for (const it of items) {
    ...
    if (packs === 0) continue;
    if (!Number.isInteger(packs) || packs < 0) return { ok: false, error: `${it.sku}: packs returned must be a whole number.` };
    if (packs > it.quantity) return { ok: false, error: `${it.sku}: returned packs cannot exceed the ${it.quantity} shipped.` };
    if (!it.lotId) return { ok: false, error: `${it.sku} has no lot recorded; the shipment record is incomplete.` };
    lines.push({ itemId: it.id, packs });
    refundDueCents += packs * it.unitPriceCents;
  }
  if (lines.length === 0) return { ok: false, error: 'Enter how many packs came back on at least one line.' };
  return { ok: true, lines, receivedOn, condition, note: (raw.note ?? '').trim().slice(0, 300), refundDueCents };
}
