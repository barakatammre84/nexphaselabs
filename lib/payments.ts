...
const bankTransfer: PaymentMethod = {
  ...
  enabled: () => Boolean(env.PAYMENT_BANK_INSTRUCTIONS?.trim()),
  async begin(order) {
    return {
...
const btcpay: PaymentMethod = {
  ...
  enabled: () => Boolean(env.BTCPAY_HOST && env.BTCPAY_STORE_ID && env.BTCPAY_API_KEY && env.BTCPAY_WEBHOOK_SECRET),
  async begin(order) {
...
export function btcpayCheckoutUrl(invoiceId: string): string | null {
  if (!env.BTCPAY_HOST || !/^[A-Za-z0-9]{6,64}$/.test(invoiceId)) return null;
