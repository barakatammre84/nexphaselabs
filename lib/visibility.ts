...
export type Viewer = { account: AccountPrincipal | null; visibility: Visibility };
...
export async function currentViewer(): Promise<Viewer> {
  ...
  try {
    ...
    console.error('[viewer] account lookup failed', error instanceof Error ? error.message : error);
  }
