...
import { lotInventory, movementsForConsignee, revenueByProduct } from '@/lib/reports';
...
type Props = { searchParams: Promise<{ consignee?: string }> };
...
export default async function ReportsPage({ searchParams }: Props) {
  ...
  const { consignee } = await searchParams;
  const filter = (consignee ?? '').slice(0, 80);

  const inventory = admin ? await loadCatalog(lotInventory) : null;
  const revenue = admin ? await loadCatalog(revenueByProduct) : null;
  const movements = admin ? await loadCatalog(() => movementsForConsignee(filter)) : null;
                  ...
                  {(movements?.data ?? []).length === 0 ? (
                    ...
                    (movements?.data ?? []).map((m, i) => (
                      <tr key={`${m.lotNumber}-${i}`} className="border-b border-border last:border-b-0">
                        ...
                        <td className={`${td} font-mono text-xs`}>{m.quantity}</td>
                        <td className={td}>
                          {m.consigneeName ?? '—'}
