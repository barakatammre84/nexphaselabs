...
export default async function DashboardPage({ searchParams }: Props) {
  ...
  const tiles = c
    ? [
        { n: c.verificationsWaiting, label: 'organisations waiting for a decision', href: '/manage/verification', urgent: c.verificationsWaiting > 0 },
        { n: c.ordersAwaitingPayment, label: 'orders awaiting payment', href: '/manage/orders', urgent: false },
        { n: c.ordersToFulfil, label: 'paid orders to pick and ship', href: '/manage/orders', urgent: c.ordersToFulfil > 0 },
        { n: c.ordersFulfilling, label: 'orders being prepared', href: '/manage/orders', urgent: false },
        { n: c.refundsDue, label: 'refunds due', href: '/manage/orders', urgent: c.refundsDue > 0 },
        { n: c.lotsInQuarantine, label: 'lots in quarantine', href: '/manage/lots', urgent: false },
    ...
    <main className="bg-background text-foreground">
      <section className="mx-auto max-w-[1500px] px-5 py-14 sm:px-8 lg:px-12">
        ...
        <div className="mt-6 flex flex-wrap items-end justify-between gap-6">
          <h1 className="font-display text-[clamp(2.2rem,4.2vw,3.6rem)] font-extrabold leading-[0.95] tracking-[-0.05em]">What needs a person</h1>
          {admin && (
            <form method="post" action="/api/manage/digest">
              <button type="submit" className="inline-flex h-11 items-center gap-2 border border-foreground/20 px-5 text-sm font-semibold hover:border-primary hover:text-primary">
        ...
        {digest === 'sent' && (
          <p role="status" className="mt-6 flex items-center gap-2 border border-border bg-secondary p-4 text-sm">
            <CircleCheck className="size-4 text-primary" /> Digest sent to {staff.email}.
          </p>
        )}
        {digest === 'failed' && (
        ...
        {loaded.unavailable ? (
          ...
          <>
            <div className="mt-8 grid gap-px bg-border sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {tiles.map((t) => (
                <Link key={t.label} href={t.href} className="group flex flex-col justify-between gap-6 bg-background p-6 hover:bg-accent">
                  <span className={`font-display text-4xl font-extrabold tracking-tight ${t.urgent ? 'text-primary' : t.n === 0 ? 'text-muted-foreground' : ''}`}>{t.n}</span>
            ...
            <div className="mt-12 flex items-end justify-between gap-6">
              ...
              <Link href="/manage/activity" className="text-sm font-semibold text-primary hover:underline">
                Full timeline
              </Link>
            </div>
            <ul className="mt-4 divide-y divide-border border border-border text-sm">
              {(loaded.data?.recent ?? []).length === 0 && <li className="p-3 text-muted-foreground">Nothing recorded yet.</li>}
