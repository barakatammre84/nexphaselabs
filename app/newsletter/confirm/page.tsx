...
export default async function NewsletterConfirmPage({ searchParams }: Props) {
  ...
  try {
    ...
    reportServerFailure('newsletter-confirm');
    outcome = 'unavailable';
  }
  ...
  return (
    <main className="text-foreground">
      <section className="ion-panel mx-auto my-12 max-w-2xl px-7 py-10 sm:px-10">
        ...
        <h1 className="ion-heading mt-4 text-3xl">
        ...
        >
          {ok ? (
            ...
            <AlertCircle className="mt-1 size-4 text-destructive" />
          )}
          <span>{message}</span>
        </p>
        ...
        <Link href="/catalog" className="action-primary mt-8">
          Browse the catalog
        </Link>
      </section>
    </main>
