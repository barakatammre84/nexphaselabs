import { issueInvoice, previewInvoice } from '@/lib/invoice';
...
/**
 * Preview and issue the invoice for an order.
 *
 * GET renders the document and stores nothing. POST claims the number, writes
 * the PDF and records the issue.
 *
 * Invoices are order work, so this is gated on `canFulfil` (admin or ops)
 * rather than the QC permission that governs the analytical record.
 */
...
export async function GET(
  ...
  let preview;
  try {
    preview = await previewInvoice(normalised);
  } catch (error) {
    console.error(
  ...
  const bytes = await renderInvoice(preview.content, {
...
export async function POST(
  ...
  try {
    const result = await issueInvoice(normalised, staff, { reason });
    if (!result.ok) {
      console.info(
        `[invoice] issue refused for ${normalised}: ${result.errors.join('; ')}`,
      );
      return back('error=invoiceblocked');
    }
    console.info(
      `[invoice] ${result.documentNumber} issued for ${normalised} by ${staff.id}`,
    );
    return back(`invoiced=${encodeURIComponent(result.documentNumber)}`);
  } catch (error) {
    console.error(
      '[invoice] issue failed',
      error instanceof Error ? error.message : error,
    );
    return back('error=invoicefailed');
  }
}
