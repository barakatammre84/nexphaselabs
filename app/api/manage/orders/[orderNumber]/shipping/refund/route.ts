...
export async function POST(
  ...
  if (!(await allow(rateLimitKey('shipping-refund', detail.order.id), 5, 3600)))
    return reply(
      {
        error:
          'Label cancellation request limit reached. Reconcile the existing request.',
      },
      429,
    );
  let form: FormData;
  ...
  try {
    const result = await voidShippingLabel(
      detail.order,
    ...
    console.error(
      '[shipping-refund] failed',
      error instanceof Error ? error.message : error,
    );
    return reply(
      {
        error:
          'Label cancellation was not confirmed. Do not retry; reconcile it in Shippo.',
      },
      409,
    );
  }
