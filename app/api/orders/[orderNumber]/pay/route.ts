...
export async function POST(request: Request, { params }: { params: Promise<{ orderNumber: string }> }) {
  ...
  try {
    const result = await beginPayment(detail, method, account.email, `${account.name} (${account.id})`);
    if (!result.ok) return back(`error=${encodeURIComponent(result.error)}`);
  } catch (error) {
    console.error('[orders] pay failed', error instanceof Error ? error.message : error);
