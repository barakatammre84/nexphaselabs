...
export async function POST(request: Request) {
  ...
  const quantity = form.get('remove') === '1' ? 0 : parseQuantityInput(String(form.get('quantity') ?? ''));
  if (!/^cit_[a-z0-9]{8,32}$/.test(itemId) || quantity === null) {
    return Response.redirect(new URL('/account/cart?error=invalid', request.url), 303);
  }
  try {
    await setCartQuantity(account.id, itemId, quantity);
  } catch (error) {
    console.error('[cart] update failed', error instanceof Error ? error.message : error);
    return Response.redirect(new URL('/account/cart?error=unavailable', request.url), 303);
  }
