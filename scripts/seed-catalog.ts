...
import { DEFAULT_PRESENTATION, validateProductInput, type ProductInput } from '../lib/catalog-rules';
...
const statements: string[] = ['-- Generated by scripts/seed-catalog.ts. Do not edit by hand.'];
let sortOrder = 0;
...
for (const product of seedProducts) {
  ...
  if (!result.ok) {
    ...
    console.error(JSON.stringify({ errors: result.errors, violations: result.violations }, null, 2));
    process.exit(1);
  }
...
console.log(`Wrote drizzle/seed/catalog.sql (${seedProducts.length} products, ${statements.length - 1} statements).`);
