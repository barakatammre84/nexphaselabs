...
}, (table) => ({ dueIdx: index('notifications_due_idx').on(table.status, table.nextAttemptAt) }));
...
export const notificationEvents = sqliteTable('notification_events', {
  ...
  detail: text('detail'),
  createdAt: integer('created_at', { mode: 'timestamp' }).notNull().default(sql`(unixepoch())`),
}, (table) => ({ notificationIdx: index('notification_events_notification_idx').on(table.notificationId) }));
